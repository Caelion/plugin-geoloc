...
class FindMyPhone {
	...
	public function __construct($username, $password) {
		if (!extension_loaded('curl')) {
			throw new FindMyPhoneException('PHP extension cURL is not loaded.');
		}
	...
	public function play_sound($device_id, $subject = 'Find My iPhone Alert') {
		if(!is_string($device_id)) throw new FindMyPhoneException('Expected $device_id to be a string');
		if(!is_string($subject)) throw new FindMyPhoneException('Expected $subject to be a string');
	...
	public function send_message($device_id, $text, $sound = false, $subject = 'Important Message') {
		if(!is_string($device_id)) throw new FindMyPhoneException('Expected $device_id to be a string');
		if(!is_string($text)) throw new FindMyPhoneException('Expected $text to be a string');
		if(!is_bool($sound)) throw new FindMyPhoneException('Expected $sound to be a bool');
		if(!is_string($subject)) throw new FindMyPhoneException('Expected $subject to be a string');
	...
	public function lost_device($device_id, $passcode, $owner_phone_number = '911', $sound = true, $text = 'This iPhone has been lost. Please call me.') {
		if(!is_string($device_id)) throw new FindMyPhoneException('Expected $device_id to be a string');
		if(!is_string($passcode)) throw new FindMyPhoneException('Expected $passcode to be a string');
		if(strlen($passcode) !== 4) throw new FindMyPhoneException('Expected $passcode to be 4 characters long');
		if(!is_string($owner_phone_number)) throw new FindMyPhoneException('Expected $owner_phone_number to be a string');
		if(!is_bool($sound)) throw new FindMyPhoneException('Expected $sound to be a bool');
		if(!is_string($text)) throw new FindMyPhoneException('Expected $text to be a string');
	...
	public function notify_when_found($device_id, $notify = true) {
		if(!is_string($device_id)) throw new FindMyPhoneException('Expected $device_id to be a string');
		if(!is_string($notify)) throw new FindMyPhoneException('Expected $notify to be a boolean');
	...
	public function lock_and_message($device_id, $passcode, $text, $sound = true, $title = 'Find My iPhone Alert') {
		...
		if(!is_string($text)) throw new FindMyPhoneException('Expected $text to be a string');
		if(!is_bool($sound)) throw new FindMyPhoneException('Expected $sound to be a bool');
		if(!is_string($title)) throw new FindMyPhoneException('Expected $title to be a string');
	 ...
	 * 
	 */
	public function locate_device($device, $timeout = 50) {
		if(!is_integer($device)) throw new FindMyPhoneException('Expected $device to be an integer');
		...
		while (!$this->devices[$device]->location->locationFinished) {
			if ((time() - $start) > intval($timeout)) {
				log::add('geoloc','debug','locationFinished:False : Failed to locate device! Request timed out.');
				break 1;
			}
			sleep(5);
			$this->refresh_client();
	...
	private function make_request($method, $post_data, $return_headers = false, $headers = array()) {
		if(!is_string($method)) throw new FindMyPhoneException('Expected $method to be a string');
		if(!$this->is_json($post_data)) throw new FindMyPhoneException('Expected $post_data to be json');
		if(!is_array($headers)) throw new FindMyPhoneException('Expected $headers to be an array');
		if(!is_bool($return_headers)) throw new FindMyPhoneException('Expected $return_headers to be a bool');
		...
		curl_close($curl);
log::add('geoloc','debug',$http_result);
		return $http_result;
	}
...
class FindMyPhoneException extends Exception {}
